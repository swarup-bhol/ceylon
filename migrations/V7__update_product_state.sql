...
union all
select i.product_id,c.state, i.qty,i.cts,c.due_date,i.price_per_carat, i.type as item_type from consignment_items inner join invoiced_item i on i.id = consignment_items.items_id inner join consignment c on c.id = consignment_items.consignment_id where c.state ='AWAY' or c.state = 'OVERDUE';
